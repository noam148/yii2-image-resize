...
class ImageResize
{
    ...
    /**
     * @var string $imageExtension the target file extension is used to determine file format, only jpg, jpeg, gif, png, wbmp, xbm, webp and bmp are supported.
     */
    public $imageExtension = 'jpg';
    ...
    public function generateImage($filePath, $width, $height, $mode = "outbound", $quality = null, $chosenFileName = null)
    {
        ...
        // validate extension
        if ($this->imageExtension !== null && !in_array($this->imageExtension, ['jpg', 'jpeg', 'gif', 'png', 'wbmp', 'xbm', 'webp', 'bmp'])) {
            throw new Exception("generateImage $this->imageExtension is not valid, only jpg, jpeg, gif, png, wbmp, xbm, webp and bmp are supported");
        }
        ...
        foreach ($this->cachePath as $cachePath) {
            ...
            $imageFile = $imageFilePath . DIRECTORY_SEPARATOR . $imageFileName . '.' . $this->imageExtension;
